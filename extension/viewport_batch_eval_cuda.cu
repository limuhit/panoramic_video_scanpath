...
__global__ void vp_eval_bt_init_xyz_kernel(int num, scalar_t * data, int height, int width, float w_stride, float h_stride, float c_x, float c_y){
...
template <typename scalar_t>
__global__ void bt_eval_rotation_kernels(const int nthreads, const scalar_t* const theta_phi, scalar_t * const r, 
...
template <typename scalar_t>
__global__ void vp_eval_bt_transpose_kernel(int num, const scalar_t * const x, const scalar_t * y, scalar_t * const z, 
...
void viewport_batch_eval_opt::init(){
    init_base();
    height_ = -1;
    width_ = -1;
...
bool viewport_batch_eval_opt::reshape(int num, int channel, int height, int width){
     return reshape_base(num, channel, height, width); 
}

void viewport_batch_eval_opt::reshape_top(at::TensorOptions option){
    ...
    shapes.push_back({batch_, npred_, 2});
    reshape_top_base(option,shapes);
}

void viewport_batch_eval_opt::reshape_rota(at::TensorOptions options){
...
template <typename scalar_t>
__global__ void vp_eval_bt_cal_xyz_kernel(int num, scalar_t * const xyz, scalar_t * tf,  scalar_t hx, scalar_t hy, scalar_t pi, 
...
template <typename scalar_t>
__global__ void viewport_eval_bt_forward_kernel(const int nthreads, const scalar_t* const input,  
...
void viewport_batch_eval_opt::set_video_sequence(at::Tensor  bottom_data, int start_idx, int len){
    idx_ = start_idx;
    len_ = len;
        ...
        AT_DISPATCH_FLOATING_TYPES(
            bottom_data.scalar_type(), "viewport_forward_cuda", 
                ([&] {
                    ...
                    vp_eval_bt_init_xyz_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_>> >
...
template <typename scalar_t>
__global__ void erp2vp_eval_bt_forward_kernel(int num, const scalar_t * const tf, const scalar_t * y, scalar_t * const out,
...
template <typename scalar_t>
__global__ void target_eval_bt_forward_kernel(int num, const scalar_t * const tf, const scalar_t * y, scalar_t * const out,
     const scalar_t rad, const int start_idx,  const int len, const int wind, const int npred,  const scalar_t pi){
    CUDA_KERNEL_LOOP(i, num) {
        int pn = i / npred;
        int pw = start_idx + i % npred;
        if(pw<0 || pw>len){
            out[i*2] = 0;
            out[i*2 + 1] = 0;
            continue;
...
std::vector<at::Tensor>  viewport_batch_eval_opt::forward_cuda(at::Tensor video, at::Tensor theta_phi) 
{
	int count;
    int start_idx = idx_ - windows_;
	AT_DISPATCH_FLOATING_TYPES(
		theta_phi.scalar_type(), "viewport_batch_eval_forward_cuda", 
        ([&] {
            count = batch_* windows_;
            scalar_t spi = pi_;            
            bt_eval_rotation_kernels<<<CAFFE_GET_BLOCKS(count),CAFFE_CUDA_NUM_THREADS, 0, stream_>>>
                (count,theta_phi.data_ptr<scalar_t>(),rota_.data_ptr<scalar_t>(), len_, start_idx, windows_, spi);
            CUDA_POST_KERNEL_CHECK;
            count = batch_ * windows_ * h_out_ * w_out_;
            vp_eval_bt_transpose_kernel<<<CAFFE_GET_BLOCKS(count),CAFFE_CUDA_NUM_THREADS, 0, stream_>>>
            ...
            scalar_t hx = width_;
            scalar_t hy = height_;
            vp_eval_bt_cal_xyz_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
            ...
            viewport_eval_bt_forward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
            ...
            scalar_t rad = rad_;
            erp2vp_eval_bt_forward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
            ...
            count = batch_* windows_;
            target_eval_bt_forward_kernel<< <CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS, 0, stream_ >> >
                (count, theta_phi.data_ptr<scalar_t>(), rota_.data_ptr<scalar_t>(), top_data_[5].data_ptr<scalar_t>(),
                    rad, idx_,  len_, windows_, npred_, spi);
            CUDA_POST_KERNEL_CHECK;
        })
    );
    idx_ += npred_;
