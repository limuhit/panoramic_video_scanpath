...
class viewport_batch_opt: public base_opt{
	public:
		viewport_batch_opt(float fov, int h, int w, int windows, int npred, int topk, int device = 0, bool timeit=false){
			pi_ = acos(-1.0);
			fov_ = fov / 180 * pi_;
			h_out_ = h;
			w_out_ = w;
		...
		at::Tensor set_path_sequence(at::Tensor old_path, at::Tensor path_data, at::Tensor fork_idxs, bool p1);
