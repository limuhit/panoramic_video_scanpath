...
void viewport_opt::reshape_rota(at::TensorOptions options, int num){
    std::vector<std::vector<int64_t>> shapes;
    shapes.push_back({num,9});
    at::cuda::set_device(device_);
    if(!init_rota_){
        rota_ = at::empty(shapes[0], options);
    }else{
        if(!is_same_shape(rota_.sizes(),shapes[0])){
            rota_ = at::empty(shapes[0], options);
        }
    }
    init_rota_ = true;
...
template <typename scalar_t>
__global__ void rotation_kernels_fake(const int nthreads, scalar_t * const r){
	CUDA_KERNEL_LOOP(index, nthreads) {
		r[index*9+0] = 1;
		r[index*9+1] = 0;
		r[index*9+2] = 0;
